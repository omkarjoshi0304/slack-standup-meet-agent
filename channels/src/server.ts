...
// a declared-but-unprovisioned Channel counts as a valid degraded state.
const status = channels.status();
if (status.overall !== "online") {
  console.error(
    `\n  Channel is not online: ${JSON.stringify(status)}\n` +
      `  → 'setup_required' means the provider side is unfinished (Slack app / channel provisioning).\n`,
  );
  await teardown();
  process.exit(1);
}
